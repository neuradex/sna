...
description: Request/response op、push チャネル、再接続ルールを整理します。
icon: Cable
---

WebSocket はライブチャネルです。push イベント (assistant delta、ツール
...
`id` フィールドはクライアントが選ぶ相関トークンです。request/response
...
- `agent.event`: 購読中の全セッションが出す全 `AgentEvent` ごとに
  発生します。`agent.subscribe` で購読し、`agent.unsubscribe` で解除します。
- `permission.request`: どのセッションが `permission_needed` を emit
  しても発生します。`permission.subscribe` で購読します。

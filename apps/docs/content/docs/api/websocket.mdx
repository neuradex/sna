...
WebSocket is the live channel. It carries push events (assistant
deltas, tool calls, permission requests) and the same control ops the
...
- `agent.event`: fires for every `AgentEvent` produced by every
  subscribed session. Subscribe with `agent.subscribe`, unsubscribe
  with `agent.unsubscribe`.

---
title: 프로바이더
description: AgentProvider 인터페이스, runtime pool, 프로바이더별 특이점.
icon: Boxes
---

각 런타임은 공통 `AgentProvider` 인터페이스 뒤에 클래스로 구현됨.
...
respawn-with-history-replay가 필요한지 결정한다.
...
Codex와 OpenCode 프로바이더는 `RuntimePool`을 통해 데몬을 공유한다.
세션이 열릴 때 `(provider, cwd, config)` 키로 풀이 조회되고, 호환되는
기존 데몬이 있으면 재사용해서 cold-start 비용을 아낀다 (Codex 약 2초,
OpenCode 약 3초).
...
Claude Code는 풀링하지 않음. CLI 자체가 무상태라서, 일반적인 세션
세분도에선 호출 단위 spawn이 데몬 resume과 경쟁력이 있다.
...
- **`reasoningLevel: 0..5`**: 프로바이더 비종속, 가장 가벼움부터 가장
  ...
  [reasoning-level.ts](https://github.com/neuradex/sna/blob/main/packages/core/src/core/providers/reasoning-level.ts) 가 번역.
- **`providerOptions.serviceTier`**: Codex 전용. Codex의 `/fast`
  슬래시 명령을 모방함 (값: `"priority"`, `"flex"`, `"batch"`).
  Claude로는 의도적으로 자동 매핑 안 함. Claude의 `/fast`는 별도
  과금 풀을 가진 다른 MODEL variant이기 때문.

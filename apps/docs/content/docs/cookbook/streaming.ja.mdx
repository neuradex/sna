...
SNA は単発呼び出し向けのストリーミングを 3 つの形で公開します。ホスト
...
Promise はやはり最終的に結合されたテキストと使用量で resolve します。
`onDelta` は side channel であり、最終結果の置き換えではありません。

`claude-code` と `codex` (プールと ephemeral の両方) で配線済みです。
OpenCode の SDK 呼び出しは現状 single-shot なので、コールバックは
そこでは文書化されている no-op です。
...
非テキストイベントが不要な場合は、`onDelta` 専用の変種も使えます。
...
`POST /agent/run-once/stream` (Server-Sent Events) で実装されています。

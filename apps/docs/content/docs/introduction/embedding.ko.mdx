...
fork합니다. 호스트에 따라 두 가지 진입점을 사용할 수 있습니다.
...
Electron용 진입점은 asar-unpacked 경로를 해석하고, 컨슈머 앱이
...
- `node_modules/@sna-sdk/core/**`를 electron-builder의 `asarUnpack`에
  추가합니다. 서버의 `dist/`가 런타임에 도달 가능해야 합니다.
...
Renderer 프로세스는 임베드된 서버를 일반 HTTP/WS 호스트처럼 다룹니다.
...
따라서 React 앱에서는 IPC 없이도 renderer가 포트를 자동으로 발견할 수
있습니다.

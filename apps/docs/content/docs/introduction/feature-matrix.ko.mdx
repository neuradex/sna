...
프로바이더가 무시하거나, respawn으로 fallback하거나, 대응되는 네이티브 표면이
없다는 뜻입니다.
...
Claude Code, Grok Build, Cursor는 호출 또는 세션 단위로 무상태입니다. spawn마다
...
두 ACP-speaking 프로바이더(Grok Build, Cursor)는
...
두 ACP-speaking 프로바이더는 모든 필드를 leftover로 반환합니다. ACP에는

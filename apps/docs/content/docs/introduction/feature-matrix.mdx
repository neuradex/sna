...
| Unknown/new tool visibility | ✓ native tool name still surfaces | ✓ fail-open generic `tool_use` / `tool_result` with `raw` payload | ✗ unhandled OpenCode event types are dropped | ✗ unknown ACP update types are dropped | ✗ unknown ACP update types are dropped |
...
| `profile` | — | ⚠ pool-key isolation only; the adapter does not activate a profile | — | — | — |
| `config` (`-c key=value`) | — | ✗ not wired in the current adapter | — | — | — |
| `hooksHash` / `configHash` | — | ✓ runtime pool isolation keys | — | — | — |

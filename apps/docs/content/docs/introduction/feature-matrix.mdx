...
> `core/providers/acp/base.ts`, and the provider test suite. Runtime notes
...
Every provider emits the same `AgentEvent` union, but the native stream
...
| Session completion metadata | ✓ duration, API duration, cost, token usage, cache usage, model, context window | ✓ duration and token/cache usage; cost is not reported | ⚠ session stream only says provider; one-shot `complete()` returns usage/cost when OpenCode reports it | ✓ ACP `stopReason` only | ✓ ACP `stopReason` only |
...
tool arguments. The ACP providers expose useful `tool_call_update`
...
- **Plan mode is not uniform**: Claude Code supports native plan mode,
  ...
  wire a plan-mode flag today. ACP providers still need respawn-with-history

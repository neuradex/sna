---
title: 프로바이더
description: AgentProvider 인터페이스, runtime pool, 프로바이더별 차이를 정리합니다.
icon: Boxes
---
...
각 프로바이더가 어떤 필드를 인플레이스로 적용할 수 있고, 어떤 필드는
...
와이어 프로토콜로 쓴 프로바이더였습니다. Cursor가 두 번째 ACP-speaking
...
모든 프로바이더에 걸쳐 쓸 수 있는 두 가지 레이턴시 설정:

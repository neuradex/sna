...
| `config.provider` | `string` | no | `claude-code`、`codex`、`opencode`、`omlx` などの runtime id。 |
...
| Field | Type | Description |
| --- | --- | --- |
| `runtime` | `string` | 調べるランタイム。例: `claude-code`、`codex`、`opencode`、`omlx`。 |
| `config.cliPath` | `string` | ランタイム CLI path override。 |
| `config.baseUrl` | `string` | OMLX-style ランタイムの API base URL。 |
| `config.apiKey` | `string` | OMLX-style ランタイムの API key。 |

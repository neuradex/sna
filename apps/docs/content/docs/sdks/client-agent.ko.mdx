...
Codex에서는 `providerOptions`로 `serviceTier`, `profile`, `config`를 지정할 수 있습니다. `profile`은 `--profile <name>`으로 전달되고, `config`는 `codex app-server`와 `codex exec` 양쪽에 반복 가능한 `-c key=value` override로 전달됩니다.

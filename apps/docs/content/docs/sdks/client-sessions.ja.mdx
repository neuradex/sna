...
session record、保存済み履歴、runtime chain、保留中の権限要求を削除します。agent process が存在する場合は、削除前に停止されます。`default` session は削除できません。削除済み session への後続呼び出しは no-session または no-active-session エラーを返します。

...
| `cwd` | `dirname(dbPath)` | Server process와 provider session의 working directory. |
| `maxSessions` | `5` | 동시에 유지할 수 있는 agent session 수. |
| `permissionMode` | SDK default | 새 agent session의 기본 permission mode. |
| `model` | SDK default | Provider call의 기본 model. |
...
| `omlxBaseUrl` | none | Claude-compatible call을 oMLX endpoint로 route합니다. |
...
`stop()`은 session을 종료하고, provider runtime을 dispose하고, tracing을 flush하고, logger callback을 정리한 뒤 HTTP server를 닫습니다. Host shutdown path에서 호출하십시오.

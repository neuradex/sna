...
| `@sna-sdk/core/node`, `@sna-sdk/core/electron` | `startSnaDaemon(options)` | Starts a detached background SNA server with pid/log files and adoption. |
...
`startSnaDaemon()` accepts the same runtime, database, model, permission, and
timeout fields, plus daemon-specific file locations:
...
| `daemonDir` | `path.join(cwd, ".sna")` | Daemon metadata directory. Used by `startSnaDaemon()` only. |
| `pidPath` | `<daemonDir>/sna-daemon.pid` | PID file path for the detached server. |
| `logPath` | `<daemonDir>/sna-daemon.log` | Combined stdout/stderr log file for the detached server. |
| `adoptExisting` | `true` | If `/health` on the requested port already reports SNA, return an adopted handle instead of spawning another server. |
| `healthTimeout` | `1000` | Per-request timeout for daemon `/health` checks. |
| `stopTimeout` | `5000` | Graceful stop timeout before the daemon launcher sends `SIGKILL`. |
...
| Detached `startSnaDaemon` | You want SNA to keep running as a background local server after the launcher returns. | The SDK manages pid/log/status/adoption, but it is not an OS service manager; use launchd, systemd, or your app supervisor if you need login-time or crash-time restart. |
...
The daemon serves the admin shell from the same API origin at `/admin`. Loading
that shell sets an HttpOnly same-origin admin cookie, so browser admin requests
do not need the owner bearer token in localStorage or in the URL.

For repository development, the `mise.toml` tasks provide a repeatable smoke
flow:
...
Use the Vite admin console only through mise so its dev proxy injects the local
daemon owner token:
...
The local PKCE authorization flow can be tested manually:
...
`sna:pkce:check` verifies that the app token can access scoped session/chat
routes and is denied from owner-only agent routes. `mise run sna:auth-smoke`
runs the automated auth/OpenAPI/WebSocket/daemon test slice.
...
### `startSnaDaemon(options)` from `@sna-sdk/core/node` or `@sna-sdk/core/electron`

Starts the same standalone SNA server as a detached background process. The
launcher waits for `/health`, writes `.sna/sna-daemon.pid`,
`.sna/sna-daemon.log`, and `.sna/sna-api.port`, then returns a small control
handle.
...
If a healthy SNA daemon is already serving the requested port, the launcher
returns `adopted: true` and does not take ownership of that process. In that
case `stop()` returns `false`.

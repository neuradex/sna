...
Use the Codex runtime for OpenAI-compatible gateways. SNA does not expose a
runtime-neutral `apiBaseUrl`, because the supported endpoint override surface
differs by runtime. Codex already has a native model-provider configuration
surface, so SNA forwards it through `providerOptions.config`.

The endpoint must match the wire API supported by your installed Codex CLI.
Codex `0.132.0` expects `wire_api = "responses"` for custom providers; a
legacy chat-completions-only endpoint is not enough for this path.
...
For a local model server, keep the same shape and replace the provider id,
`base_url`, `env_key`, and `model` with values accepted by that server. Do
not route OpenRouter or local OpenAI-compatible servers through Claude Code's
Anthropic environment variables; that bypasses the Codex harness SNA is meant
to preserve.

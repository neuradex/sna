...
`SessionManager`는 server-side session record, provider runtime, event buffer, persisted history, lifecycle callback을 조율합니다.
...
| Runtime session | Session에 현재 붙어 있는 provider process 또는 runtime handle. Config 변경으로 respawn이 필요하면 runtime chain이 생길 수 있습니다. |

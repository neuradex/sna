...
| `provider` | `claude-code` | start/completion に使う provider。 |
| `permissionMode` | provider default | tool permission mode。 |
| `reasoningLevel` | provider default | provider-neutral reasoning effort。 |
| `providerOptions` | none | provider-specific option。Codex `serviceTier` などを渡せます。 |
...
| Provider default | `provider` のデフォルトは `claude-code` です。 |
...
| `completion(opts)` | `POST /agent/completion` | Hook の provider と reasoning default を使って stateless completion request を送ります。 |

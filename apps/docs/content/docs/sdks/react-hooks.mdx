...
| Provider default | `provider` defaults to `claude-code`. |
...
| `completion(opts)` | `POST /agent/completion` | Sends a stateless completion request using the hook provider and reasoning defaults. |

...
export function ChatPanel({ onClose, sessionId: initialSessionId = "default" }: ChatPanelProps) {
  ...
  // Build background session list — merge chat store + server state
  const bgSessions = Object.entries(allSessions)
    ...
    .map(([id, sess]) => {
      ...
      const lastMsgStatus = (lastMsg?.meta?.status as string) ?? "";
      ...
      >
        ...
        {viewMode === "bg-dashboard" && (
          ...
          >
            ...
            {bgSessions.map((bg) => {
              const statusColor = bg.status === "running" ? "var(--sna-accent)"
                : bg.status === "complete" ? "var(--sna-success)"
                : bg.status === "failed" ? "var(--sna-error, #ef4444)"
                : "var(--sna-text-faint)";
              const statusLabel = bg.status === "running" ? "Running"
                : bg.status === "complete" ? "Complete"
              ...
              return (
                ...
                >
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                    ...
                    />
                    <span style={{ fontWeight: 500, flex: 1 }}>{bg.label}</span>
                    <span style={{
                      fontSize: 9,
                      padding: "2px 6px",
                      borderRadius: "var(--sna-radius-full)",
                  ...
                  {bg.lastMessage && (
                    <div style={{ color: "var(--sna-text-faint)", fontSize: 11, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", marginBottom: 8 }}>
                      {bg.lastMessage}
                    </div>
                  )}
                  {/* Action buttons */}
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <button
                      ...
                      style={{
                        padding: "3px 8px", borderRadius: "var(--sna-radius-sm)",
                        background: "var(--sna-surface-hover)", border: "none",
                        color: "var(--sna-text-muted)", cursor: "pointer",
                    ...
                    {bg.alive && (
                      <button
                        onClick={() => { killServerSession(bg.id); setTimeout(refreshSessions, 500); }}
                        style={{
                          padding: "3px 8px", borderRadius: "var(--sna-radius-sm)",
                    ...
                    <button
                      onClick={() => { removeSession(bg.id); deleteServerSession(bg.id); }}
                      style={{
                        padding: "3px 8px", borderRadius: "var(--sna-radius-sm)",
                        background: "none", border: "none",
                        color: "var(--sna-text-faint)", cursor: "pointer",
                        fontSize: 10, fontFamily: "inherit", marginLeft: "auto",
                      }}
                    >
                      Delete
                    </button>

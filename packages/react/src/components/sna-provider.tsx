...
export function SnaProvider({
  ...
  // 1. Discover SNA API port (always runs — headless apps need the URL too)
  useEffect(() => {
  ...
  // 2. Auto-open chat on first mount (skip in headless)
  useEffect(() => {
  ...
  // 3. Keyboard shortcut: Cmd/Ctrl + . to toggle chat (skip in headless)
  useEffect(() => {
    if (headless) return;
  ...
  // Headless mode: context only, no UI
  if (headless) {
    return (
